/** Adaptive tutor chat: explains concepts, wrong answers, and study strategy. */
...
export function Tutor({ overview }: { overview: Overview }) {
  ...
  return (
    <div className="page" style={{ maxWidth: 760, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
        {SUGGESTIONS.map((s) => (
          <button key={s} className="btn btn-secondary btn-sm" disabled={busy} onClick={() => void send(s)}>
            {s}
          </button>
        ))}
      </div>
