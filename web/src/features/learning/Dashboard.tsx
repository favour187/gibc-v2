/** Dashboard: progress, next skill, review queue, mastery overview. */
      ...
      <Card>
        ...
        <div style={{ display: "grid", gap: 8 }}>
          {overview.frontier.map((sid) => {
            ...
            return (
              <div key={sid} className="spread" style={{ padding: "10px 12px", border: "1px solid var(--border)", borderRadius: 10 }}>
                <div>
      ...
      <Card className="stack">
        ...
        <div className="row">
          <select className="input" style={{ maxWidth: 320 }} value={target} onChange={(e) => setTarget(e.target.value)}>
            ...
            {overview.curriculum.skills.map((s: SkillDTO) => (
...
function ErrorBannerUI({ message }: { message: string }) {
