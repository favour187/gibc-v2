/** Skill map: the prerequisite DAG, rendered as columns by unlock order. */
...
export function PathMap({
  ...
  // Organise skills into columns = order index (simple, readable layout).
  ...
  return (
    <div className="page">
      <div className="spread">
        ...
        {overview.target_skill && (
          <Badge tone="accent">target: {byId.get(overview.target_skill)?.title ?? overview.target_skill}</Badge>
        )}
      </div>

      <div style={{ display: "grid", gridTemplateColumns: `repeat(${columns.length}, minmax(150px, 1fr))`, gap: 12, overflowX: "auto" }}>
        {columns.map((col, i) => (
          <div key={i} className="stack" style={{ alignContent: "start" }}>
            ...
            {col.map((skill) => {
              ...
              const locked =
                ...
                skill.prerequisites.some(
                  (p) => !(overview.state_map[p] && overview.state_map[p].mastery >= 70)
                );
              return (
                <button
                  key={skill.skill_id}
                ...
                >
                  ...
                  <span className="row" style={{ gap: 4 }}>
                    ...
                    {state?.mastered ? (
                      <Badge tone="success">{Math.round(state.mastery)}% ✓</Badge>
                    ) : state ? (
                      <Badge tone="accent">{Math.round(state.mastery)}%</Badge>
                    ) : (
                      <Badge tone="neutral">{locked ? "locked" : "new"}</Badge>
                    )}
                  </span>
                </button>
              );
      ...
      >
        {selected && (
          <div className="stack">
            ...
            {overview.state_map[selected.skill_id] && (
              <ProgressNote
                mastery={overview.state_map[selected.skill_id].mastery}
                encounters={overview.state_map[selected.skill_id].encounters}
                due={overview.state_map[selected.skill_id].due_date}
              />
            )}
          </div>
        )}
...
function ProgressNote({ mastery, encounters, due }: { mastery: number; encounters: number; due: string }) {
  return (
