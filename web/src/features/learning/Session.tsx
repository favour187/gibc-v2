/** Adaptive quiz session: one question at a time, instant mastery feedback. */
...
export function Session({ onFinished }: { onFinished: () => void }) {
  ...
  async function submit() {
    ...
    try {
      ...
      setResult(res);
      if (res.record.correct) setCorrectCount((c) => c + 1);
      setPhase("feedback");
    } catch (err) {
      setError((err as Error).message);
    }
  ...
  return (
    <div className="page" style={{ maxWidth: 720, margin: "0 auto" }}>
      ...
      <Card className="stack">
        ...
        <div className="stack">
          {question.options.map((option, i) => {
            return (
              <button
                key={i}
                className={cx("btn", chosen === i ? "btn-primary" : "btn-secondary", "btn-lg")}
                style={{ justifyContent: "flex-start", width: "100%", whiteSpace: "normal" }}
        ...
        {phase === "question" ? (
          <Button size="lg" disabled={chosen === null} onClick={() => void submit()}>
            Check answer
          </Button>
        ) : result ? (
          <div className="stack">
            <div
              className={cx("error-banner", result.record.correct && "correct-banner")}
              style={
            ...
            >
              ...
              {!result.record.correct && result.question.misconception && (
