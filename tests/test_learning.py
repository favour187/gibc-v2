"""Tests for Adaptive Learning Studio: mastery math, path engine, API flow."""
...
def test_hard_correct_answer_is_more_informative():
    """Getting a hard question right is bigger evidence (Elo-style updates)."""
    state = SkillState("variables")
    ...
    assert state2.theta > easy_theta  # bigger update = more information
...
def test_frontier_shrinks_as_skills_are_learned():
    ...
    assert "functions" not in front          # waits for loops
...
def test_next_best_picks_weakest_frontier():
    ...
    # The engine must pick an unmastered frontier skill, not a mastered one.
...
def test_next_best_chases_target():
    ...
    # overfitting's dependency chain should attract the choice.
...
def test_session_assembly_mixes_sources():
    ...
    # Weak/frontier material must appear (no reviews due for fresh state).
...
def test_curriculum_content_is_complete():
    # every skill has at least 2 questions; all question refs valid
    for sid, skill in SKILLS.items():
...
def test_api_learning_flow(client):
    ...
    assert "answer_index" not in first  # answers must not leak to the client
    ...
    # tutor (local deterministic fallback)
    res = client.post("/api/learning/tutor", headers=headers, json={"message": "Explain loops to me"})
    assert res.status_code == 200
    tutor = res.json()
    assert tutor["reply"]
    ...
    # target selection
    res = client.patch("/api/learning/target", headers=headers, json={"skill_id": "overfitting"})
    assert res.status_code == 200
    assert res.json()["target_skill"] == "overfitting"

    res = client.patch("/api/learning/target", headers=headers, json={"skill_id": "nope"})
...
def test_api_session_finish(client):
    ...
    res = client.post("/api/learning/session/finish", headers=headers, json={"question_count": 5, "correct_count": 4})
    assert res.status_code == 200
    body = res.json()
